 ...
 * %LICENSE% - see LICENSE
 *
 * $Id: class.servicesearch.php,v 1.5 2013-01-14 21:04:52 dkolev Exp $
 */
...
/**
 * Basic search engine data serice
 *
 * @version $Revision: 1.5 $
 * @package VESHTER
 *
 */
class CServiceSearch extends CServiceWeb
{
...
{
$error_msg = "This script requires PHP 4.0.5+ with mnoGoSearch extension";
$error_made = true;
}
else
{
$main_mysql_user_login = "front_global";
$main_mysql_user_password = "Sjk8Ilpq1kzJJD";

$SearchdbLink = mysql_connect ("localhost", $main_mysql_user_login, $main_mysql_user_password);
//connects to DB
mysql_select_db ("search", $SearchdbLink);

$SearchdbQuery = "
SELECT rec_id AS id,host_alternative,name,path FROM categories WHERE (name LIKE \"" . eregi_replace("www.", "", $_SERVER["HTTP_HOST"]) . "\")";

$SearchdbResult = mysql_query($SearchdbQuery, $SearchdbLink);
$Searchrow = mysql_fetch_array($SearchdbResult, MYSQL_ASSOC);
...
{
$error_msg = "Search engine cannot find search data";
$error_made = true;
}
mysql_close($SearchdbLink);

//resume with common veshter code
$access_groups = "all";
include ("../pagesetup/filedefs.php");
$header_regular = $header;
...
$dbaddr="mysql://$main_mysql_user_login:$main_mysql_user_password@localhost/search/?dbmode=multi";
...
$temp = strftime('%a, %d %b %Y %H:%M:%S %Z',$temp);
...
$PAGETITLE = "$PAGETITLE";
$PAGETITLE.=($query_orig) ? ": " . HtmlSpecialChars(StripSlashes($query_orig)):"";

include ($header_regular);
...
if (!$found)
{
print ("Search Time: $searchtime seconds<br>Search results:\n");
...
print("Search Time: $searchtime seconds<br>Search results: <small>$wordinfo</small><HR>\n");
...
$lastmod=Udm_Get_Res_Field($res,$i,UDM_FIELD_MODIFIED);//format_lastmod(
$lastmod=($lastmod > 0) ? strtotime($lastmod):time();
$lastmod = veshter_date($lastmod, "short2");
...
if ($title && $rating && (!eregi("format=js_raw", $url)))
{

//<b>$ndoc</b>
print ("<DL><DT><a href=\"$url\" TARGET=\"_blank\"><b>$title</b></a>\n");
print ("[<b>$rating</b>]<DD>\n");
print (($desc != '')?$desc:$text."... ");//<BR>	$t_DY\n
//print ("<br><A HREF=\"$url\">Click here</A>\n");//TARGET=\"_blank\"
//print ("($contype)");
print ("$docsize bytes - $lastmod");
print ("</UL></DL>\n");

//				if (Udm_Api_Version() == 30203)
//				{
//					if ((($stored_link>0) && (Udm_Check_Stored($udm_agent,$stored_link,Udm_CRC32($udm_agent,$url)))) || ($stored_link==-1))
//					{
...
if ($c_omitted)
{
print ("<p><i>In order to show you the most relevant results, we have omitted <a title=$c_omitted>some</a>");
//if ($c_omitted > 1)
print (" entries ");
//else
//	print (" entries ");
print ("very similar to the ones already displayed.</i>");
}

if ((Udm_Api_Version() == 30203) && ($stored_link>0))
{
Udm_Close_Stored($udm_agent, $stored_link);
}

print ("<p><table border=0 cellpadding=0 cellspacing=1 class=td_secondary width=100%><tr><td width=100% class=td_bg>");
print ("<table cellpadding=5><tr><td><font class=font_bbase>");
print("$nav\n");
...
if ($have_spell_flag)
Udm_Free_Ispell_Data($udm_agent);
